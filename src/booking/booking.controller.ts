...
export class BookingController {
  ...
  @ApiQuery({ name: 'urgencyLevel', required: false, description: 'Filter by urgency level (low, medium, high, urgent, emergency)' })
  @ApiQuery({ name: 'truckType', required: false, description: 'Filter by preferred truck type' })
  @ApiQuery({ name: 'location', required: false, description: 'Filter by pickup or delivery location' })
  @ApiQuery({ name: 'myBookings', required: false, type: Boolean, description: 'Get only current user\'s bookings' })
  ...
  @ApiResponse({ status: 400, description: 'Invalid bid data or booking not accepting bids' })
  @ApiResponse({ status: 404, description: 'Booking not found' })
  @UsePipes(new ValidationPipe({ whitelist: true, forbidNonWhitelisted: true }))
  async createBid(
  ...
  @ApiResponse({ status: 400, description: 'Bid cannot be updated (not pending)' })
  ...
  @ApiQuery({ name: 'pickupCity', required: false, description: 'Filter by pickup city' })
  @ApiQuery({ name: 'deliveryCity', required: false, description: 'Filter by delivery city' })
  @ApiQuery({ name: 'truckType', required: false, description: 'Filter by required truck type' })
  @ApiQuery({ name: 'urgencyLevel', required: false, description: 'Filter by urgency level' })
  ...
  async searchBookings(
    ...
    if (minPrice > 0 || maxPrice < Number.MAX_SAFE_INTEGER) {
      result.bookings = result.bookings.filter(booking => 
        booking.proposedPrice >= minPrice && booking.proposedPrice <= maxPrice
      );
